...
# Function: to start, stop and restart java-vision.
...
JAR_FILE_PATH="./build/libs/java-vision.jar"
PID_FILE_PATH="java-vision.pid"
LOG_FILE_PATH="java-vision.log"
...
case "${1}" in
    start)
        ...
        if [ $? -eq 0 ]; then
            echo "Succeeded to start java-vision."
        else
            echo "Failed to start java-vision."
        fi
    ;;
    stop)
        ...
        if [ $? -eq 0 ]; then
            rm ${PID_FILE_PATH}
            echo "Succeeded to stop java-vision."
        else
            echo "Failed to stop java-vision."
        fi
    ;;
    restart)
