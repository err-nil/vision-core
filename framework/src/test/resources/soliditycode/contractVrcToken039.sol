 ...
 * 2. caller部署proxy,   传入1000 token，1000 vs
 * 3. caller部署A
 * 4. caller部署B
 * 5. caller调用proxy中upgradetTo函数，传入A的地址
 * 6. caller调用proxy中不存在的trans(uint256,address,vrcToken)函数，注意这时vrcToken是无意义的，但也带上tokenid。address是任意另外某账户的地址
 * 7. 可以看到目标地址vs增长5，caller账户vs减少5
 * 8. caller调用proxy中upgradeTo函数，传入B的地址
 * 9. caller调用proxy中不存在的trans(uint256,address,vrcToken)函数。
