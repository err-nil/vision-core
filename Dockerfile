...
RUN set -o errexit -o nounset \
    ...
    && mkdir -p $TMP_DIR \
    && cd $TMP_DIR \
    && git clone git@github.com:vworldgenesis/java-vron-voyage.git \
    && cd java-vron-voyage \
    && git checkout docker \
    && ./gradlew build -x test \
    && cd build/distributions \
    && unzip -o java-vision-1.0.0.zip \
    && mv java-vision-1.0.0 $BASE_DIR \
    && rm -rf $TMP_DIR \
    && rm -rf ~/.gradle \
