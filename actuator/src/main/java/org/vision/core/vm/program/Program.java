...
import org.vision.core.vm.program.listener.CompositeProgramListener;
...
import org.vision.protos.contract.SmartContractOuterClass.SmartContract.Builder;
...
public class Program {
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] owner = TransactionTrace.convertToVisionAddress(getContractAddress().getLast20Bytes());
    byte[] obtainer = TransactionTrace.convertToVisionAddress(obtainerAddress.getLast20Bytes());

    if (VMConfig.allowVvmStake()) {
      ...
      try {
        ...
        if (VMConfig.allowVvmConstantinople()) {
          throw new TransferException(
              "transfer all token or transfer all vs failed in suicide: %s", e.getMessage());
        }
  ...
  private void suicideFreezeBalanceAndVote(byte[] owner, byte[] obtainer, Repository repository) {
    ...
    if (FastByteComparisons.compareTo(obtainer, 0, 20,
            TransactionTrace.convertToVisionAddress(new byte[20]), 0, 20) == 0
    ...
    //vote
    {
      ...
      if (!oldVotes.isEmpty()) {
        ...
        byte[] zeroAddress = TransactionTrace.convertToVisionAddress(new byte[20]);
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
    ...
    byte[] senderAddress = TransactionTrace
        .convertToVisionAddress(this.getContractAddress().getLast20Bytes());
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = TransactionTrace
        .convertToVisionAddress(msg.getCodeAddress().getLast20Bytes());
    byte[] senderAddress = TransactionTrace
        .convertToVisionAddress(getContractAddress().getLast20Bytes());
    ...
    if (!isTokenTransfer) {
      ...
      // transfer vrc10 token validation
  ...
  public void storageSave(DataWord word1, DataWord word2) {
    ...
    getContractState()
        .putStorageValue(
            TransactionTrace.convertToVisionAddress(getContractAddress().getLast20Bytes()), keyWord,
            valWord);
  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit()
        .getCode(TransactionTrace.convertToVisionAddress(address.getLast20Bytes()));
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] vision = TransactionTrace.convertToVisionAddress(address.getLast20Bytes());
    AccountCapsule account = getContractState().getAccount(vision);
    if (account != null) {
      ContractCapsule contract = getContractState().getContract(vision);
      byte[] codeHash;
      if (contract != null) {
        ...
        if (ByteUtil.isNullOrZeroArray(codeHash)) {
          ...
          getContractState().updateContract(vision, contract);
        }
      } else {
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState()
        .getBalance(TransactionTrace.convertToVisionAddress(address.getLast20Bytes()));
  ...
  public DataWord getRewardBalance(DataWord address) {
    ...
    long rewardBalance = contractService
            .queryReward(TransactionTrace.convertToVisionAddress(address.getLast20Bytes()), getContractState());
    return new DataWord(rewardBalance);
  }

  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState()
        .getContract(TransactionTrace.convertToVisionAddress(address.getLast20Bytes()));
  ...
  public DataWord isSRCandidate(DataWord address) {
    WitnessCapsule witnessCapsule = getContractState()
            .getWitnessCapsule(TransactionTrace.convertToVisionAddress(address.getLast20Bytes()));
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    long ret = getContractState()
        .getTokenBalance(TransactionTrace.convertToVisionAddress(address.getLast20Bytes()),
  ...
  public void createContract2(DataWord value, DataWord memStart, DataWord memSize, DataWord salt) {
    ...
    if(VMConfig.allowVvmIstanbul()) {
      ...
      senderAddress = TransactionTrace
          .convertToVisionAddress(this.getCallerAddress().getLast20Bytes());
    }
    byte[] programCode = memoryChunk(memStart.intValue(), memSize.intValue());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      contract.setCallerAddress(TransactionTrace.convertToVisionAddress(msg.getType().callIsDelegate()
  ...
  private void createAccountIfNotExist(Repository deposit, byte[] contextAddress) {
    if (VMConfig.allowVvmSolidity059()) {
      //after solidity059 proposal , allow contract transfer vrc10 or VS to non-exist address(would create one)
  ...
  public boolean stake(DataWord srAddress, DataWord stakeAmount) {
    ...
    stakeParam.setSrAddress(TransactionTrace.convertToVisionAddress(srAddress.getLast20Bytes()));
  ...
  public void withdrawReward() {
    ...
    byte[] ownerAddress = TransactionTrace.convertToVisionAddress(getContractAddress().getLast20Bytes());
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(VisionException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(VisionException e) {
      return new PrecompiledContractException(e.getMessage());
    }
